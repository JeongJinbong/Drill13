...
class Zombie:
    ...
    def is_ball_less_than(self):
        if self.ball_count < play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL

    def is_ball_more_than(self):
        if self.ball_count > play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition("소년보다 공의 개수가 많은가?", self.is_ball_more_than)
        a4 = Action("소년으로 이동", self.move_to_boy)

        SEQ_chase_boy = Sequence("소년을 추적", c1, c2, a4)

        c3 = Condition("소년보다 공의 개수가 적은가?", self.is_ball_less_than)
        a5 = Action("도망치기", self.runaway_from_boy)

        SEQ_runaway = Sequence("도망", c1, c3, a5)

        SEL_chase_or_wander = Selector("추적 또는 배회", SEQ_chase_boy, SEQ_wander)

        root = SEL_chase_or_runaway_or_wander = Selector("추적 또는 도망 또는 배회", SEQ_chase_boy, SEQ_runaway, SEQ_wander)
